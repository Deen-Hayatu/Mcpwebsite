...
var policyBriefs = pgTable("policy_briefs", {
  id: serial("id").primaryKey(),
  title: text("title").notNull(),
  date: text("date").notNull(),
  excerpt: text("excerpt").notNull(),
  content: text("content").notNull(),
  type: text("type").default("brief").notNull(),
  // 'brief', 'paper', or 'opinion'
  author: text("author")
  // For opinion pieces, we can specify the author
});
var insertPolicyBriefSchema = createInsertSchema(policyBriefs).pick({
  title: true,
  date: true,
  excerpt: true,
  content: true,
...
var insertProgramSchema = createInsertSchema(programs).pick({
  title: true,
  description: true
});
...
var publications = pgTable("publications", {
  ...
  // 'policy_brief', 'research_paper', 'opinion', 'report'
...
var citations = pgTable("citations", {
  ...
  citingSource: text("citing_source").notNull(),
  // Title of citing document
  citingAuthors: text("citing_authors").array(),
  citationType: text("citation_type").notNull(),
  // 'academic', 'policy', 'media', 'government'
  citationDate: date("citation_date").notNull(),
  sourceUrl: text("source_url"),
  impactScore: integer("impact_score").default(1),
  // Weighted score based on citation type
  verified: boolean("verified").default(false),
  createdAt: timestamp("created_at").defaultNow().notNull()
});
var researchPartnerships = pgTable("research_partnerships", {
  id: serial("id").primaryKey(),
  partnerName: text("partner_name").notNull(),
  partnerType: text("partner_type").notNull(),
  // 'university', 'government', 'ngo', 'international'
...
var impactMetrics = pgTable("impact_metrics", {
  ...
  // 'policy_adoption', 'media_coverage', 'public_engagement'
  title: text("title").notNull(),
  description: text("description"),
  value: numeric("value").notNull(),
  unit: text("unit"),
  // 'count', 'percentage', 'monetary'
  sourcePublicationId: integer("source_publication_id").references(() => publications.id),
  date: date("date").notNull(),
  verificationSource: text("verification_source"),
  createdAt: timestamp("created_at").defaultNow().notNull()
});
var researchMetrics = pgTable("research_metrics", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
...
var insertPublicationSchema = createInsertSchema(publications).pick({
  title: true,
  authors: true,
  type: true,
...
var insertCitationSchema = createInsertSchema(citations).pick({
  publicationId: true,
  citingSource: true,
  citingAuthors: true,
  citationType: true,
...
var insertResearchPartnershipSchema = createInsertSchema(researchPartnerships).pick({
  partnerName: true,
  partnerType: true,
  startDate: true,
  endDate: true,
...
var insertImpactMetricSchema = createInsertSchema(impactMetrics).pick({
  metricType: true,
  title: true,
  description: true,
  value: true,
...
var insertResearchMetricSchema = createInsertSchema(researchMetrics).pick({
  name: true,
  category: true,
  value: true,
  date: true,
...
var insertEventRegistrationSchema = createInsertSchema(eventRegistrations).pick({
  eventId: true,
  name: true,
  email: true,
  phone: true,
...
var membershipApplications = pgTable("membership_applications", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  email: text("email").notNull(),
  phone: text("phone"),
...
var insertMembershipApplicationSchema = createInsertSchema(membershipApplications).pick({
  name: true,
  email: true,
  phone: true,
  membershipType: true,
...
var insertVolunteerApplicationSchema = createInsertSchema(volunteerApplications).pick({
  name: true,
  email: true,
  phone: true,
  skills: true,
...
var discussionForumRegistrations = pgTable("discussion_forum_registrations", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  email: text("email").notNull(),
...
var insertDiscussionForumRegistrationSchema = createInsertSchema(discussionForumRegistrations).pick({
  name: true,
  email: true,
  interests: true,
  policyIdeas: true,
...
var fellowshipApplications = pgTable("fellowship_applications", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  email: text("email").notNull(),
  phone: text("phone").notNull(),
...
var insertFellowshipApplicationSchema = createInsertSchema(fellowshipApplications).pick({
  name: true,
  email: true,
  phone: true,
  institution: true,
...
var studentChapterApplications = pgTable("student_chapter_applications", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  email: text("email").notNull(),
...
var insertStudentChapterApplicationSchema = createInsertSchema(studentChapterApplications).pick({
  name: true,
  email: true,
  university: true,
  studentId: true,
...
var careerApplications = pgTable("career_applications", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  email: text("email").notNull(),
  phone: text("phone").notNull(),
...
var insertCareerApplicationSchema = createInsertSchema(careerApplications).pick({
  name: true,
  email: true,
  phone: true,
  position: true,
...
var annotations = pgTable("annotations", {
  ...
  // e.g., "policy_brief", "research_paper"
  ...
  // Stores selection position data: {startOffset, endOffset, startContainer, endContainer}
  ...
  // Whether the annotation is visible to all users
  isEdited: boolean("is_edited").default(false),
  replyToId: integer("reply_to_id")
  // For threaded discussions
});
var insertAnnotationSchema = createInsertSchema(annotations).pick({
  documentType: true,
  documentId: true,
  userId: true,
  userName: true,
...
var notes = pgTable("notes", {
  ...
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  isPublic: boolean("is_public").default(false),
  // Whether the note is shared with others
...
var annotationSharing = pgTable("annotation_sharing", {
  id: serial("id").primaryKey(),
...
var insertAnnotationSharingSchema = createInsertSchema(annotationSharing).pick({
  annotationId: true,
  sharedWithEmail: true,
  shareToken: true
});
var noteSharing = pgTable("note_sharing", {
  id: serial("id").primaryKey(),
...
var insertNoteSharingSchema = createInsertSchema(noteSharing).pick({
  noteId: true,
  sharedWithEmail: true,
  shareToken: true
});
...
var staffMembers = pgTable("staff_members", {
  ...
  // JSON structure for LinkedIn, Twitter, etc.
  publications: text("publications").array(),
  isFeatured: boolean("is_featured").default(false),
  sortOrder: integer("sort_order").default(0),
  // For custom ordering on the team page
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull()
});
var insertStaffMemberSchema = createInsertSchema(staffMembers).pick({
  name: true,
  position: true,
  email: true,
  phone: true,
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/policy-briefs", async (_req, res) => {
    try {
  ...
  app2.get("/api/policy-briefs/:id", async (req, res) => {
    try {
  ...
  app2.get("/api/staff/featured", async (_req, res) => {
    try {
  ...
  app2.get("/api/research-metrics", async (_req, res) => {
    try {
  ...
  app2.get("/api/publications", async (_req, res) => {
    try {
  ...
  app2.get("/api/gallery", async (_req, res) => {
    try {
  ...
  app2.get("/api/newsletters", async (_req, res) => {
    try {
...
var routesInitializationPromise = null;
var initializeRoutes = async () => {
  if (!routesInitializationPromise) {
    routesInitializationPromise = registerRoutes(app).then(() => {
      ...
      routesInitializationPromise = null;
      throw error;
    });
  }
